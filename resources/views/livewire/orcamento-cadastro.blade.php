            ...
            <div class="tab-pane fade show active" id="cadastro" role="tabpanel" aria-labelledby="cadastro-tab">
                <div class="card">
                    <div class="card-body pt-4 p-3">
                        <form
                            action="{{ $dados['pageInfo']['form_action'] ? route($dados['pageInfo']['form_action']) : '' }}"
                            method="{{ $dados['pageInfo']['form_action'] ? $dados['pageInfo']['form_method'] : '' }}"
                            role="form text-left" id="{{ $dados['pageInfo']['id'] }}" enctype="multipart/form-data">
                            @csrf
                            <div class="row align-items-end">
                                ...
                                <div class="col-lg-9 text-end mb-4">
                                    @if ($id)
                                        <a href="{{ route('orcamentos.gerarOrcamento', ['osId' => $id]) }}"
                                            class="btn btn-gradient-primary">Gerar PDF</a>
                                        <a wire:click="sendOrcamento" class="btn btn-dark">Enviar Orçamento</a>
                                        <a wire:click="converterParaOS" class="btn bg-gradient-primary">Converter para
                                            O.S</a>
                                    @endif
                                </div>
                                ...
                                <div class="col-lg-6 text-end mt-4 tw-ml-auto"
                                    ...
                                    <h2>
                                        ...
                                        <span id="totalOrcamento">
                                            R$ <span>{{ number_format($total / 100, 2, ',', '.') }}</span>
                                        </span>
                                    </h2>
                                </div>
                            </div>
                            ...
                            <div class="d-flex justify-content-end">
                                @if ($id)
                                    <button x-on:click="$event.preventDefault()" wire:click="editOS" type="submit"
                                        class="btn bg-gradient-primary btn-md mt-4 mb-4">
                                        Editar Orçamento
                                    </button>
                                @else
                                    <button x-on:click="$event.preventDefault() ;$dispatch('saveOS')" type="submit"
                                        class="btn bg-gradient-primary btn-md mt-4 mb-4">
