<div>
    <div
        ...
        <div class="mb-3 tw-w-[50%]">
            ...
            <livewire:select-component wire:key="empresas-{{ data_get($motorista, 'id') . $vehicleCompany }}"
                target="cadastro_motorista-{{ data_get($motorista, 'id') }}" type="empresas"
                placeholder="Selecione a empresa" name="empresas[]" id="empresas" targetClass="{{ $targetClass }}"
        ...
        <div class="mb-3 tw-w-[25%] tw-pr-[8px]">
            ...
            <livewire:select-component wire:key="employeeLanguage-{{ data_get($motorista, 'id') }}" type="languages"
                target="cadastro_motorista-{{ data_get($motorista, 'id') }}" placeholder="Selecione a língua"
                name="employeeLanguage[]" id="employeeLanguage" selected='{{ $languages ?? null }}'
                targetClass="{{ $targetClass }}" />
        </div>
        <div class="mb-3 tw-w-[25%] tw-pr-[8px]">
            <label for="servico">Especialidades</label>
            <livewire:select-component wire:key="employeeSpeciality-{{ data_get($motorista, 'id') }}"
                type="especialization_general" target="cadastro_motorista-{{ data_get($motorista, 'id') }}"
        ...
        <div class="mb-3 tw-w-[25%]">
            ...
            <livewire:select-component
                ...
                type="employee_driver" target="cadastro_motorista-{{ data_get($motorista, 'id') }}"
                placeholder="Selecione o motorista" name="employee[]" id="employee" :filter="['specializations' => ['id1' => $especialization_general, 'id2' => $languages]]"
                targetClass="{{ $targetClass }}" selected='{{ $employee_driver ?? null }}'
                targetClass="{{ $targetClass }}" search="true" />
        </div>
